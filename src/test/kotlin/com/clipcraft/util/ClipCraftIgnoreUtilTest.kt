...
class FakeProject(private val basePath: String) : Project {
 ...
 override fun createError(message: String, error: Throwable?, pluginId: PluginId, attachments: MutableMap<String, String>?): RuntimeException =
...
class ClipCraftIgnoreUtilTest {
 ...
 fun `shouldIgnore based on ignorePatterns`(fileName: String, patterns: List<String>, expected: Boolean) {
  ...
  val project = FakeProject("/project")
  assertEquals(expected, ClipCraftIgnoreUtil.shouldIgnore(file, opts, project), "File: $fileName, Patterns: $patterns")
 }
 ...
 fun `shouldIgnore returns true if file is in ignoreFiles list`() {
  val opts = ClipCraftOptions(ignorePatterns = emptyList(), useGitIgnore = false, ignoreFiles = listOf("ignoreme.txt"))
 ...
 fun `complex scenario with mixed ignore patterns and gitignore merging`() {
  ...
  assertTrue(ClipCraftIgnoreUtil.shouldIgnore(file2, opts, project), "File with .secret extension should be ignored")
